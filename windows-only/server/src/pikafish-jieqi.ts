...
const toWslPath = (value: string) => {
  ...
  if (!drive || normalized[1] !== ':') return normalized
...
export const searchBestMoveWithPikafish = async (request: PikafishSearchRequest): Promise<LegalMove | null> => {
  ...
  const shouldBridgeWsl =
    process.platform === 'win32' &&
    windowsDrivePathPattern.test(request.executablePath) &&
    !/\.exe$/i.test(request.executablePath)

  const evalFilePathForEngine = request.evalFilePath
    ? shouldBridgeWsl
      ? toWslPath(request.evalFilePath)
      : request.evalFilePath
    : undefined
  ...
  const runEngine = (useSearchMoves: boolean): Promise<string | null> => {
    return new Promise<string | null>((resolve, reject) => {
      const child: ChildProcessWithoutNullStreams = shouldBridgeWsl
        ? (() => {
        const wslExecutablePath = toWslPath(request.executablePath)
        const wslDir = dirname(wslExecutablePath)
        const launchScript = `cd ${shellQuote(wslDir)} && exec ${shellQuote(wslExecutablePath)}`
        const process = spawn('wsl.exe', ['sh', '-lc', launchScript], {
          stdio: ['pipe', 'pipe', 'pipe'],
        })
        trace(`spawn via wsl executable=${wslExecutablePath}`)
        return process
      })()
        : (() => {
        const process = spawn(request.executablePath, [], {
          stdio: ['pipe', 'pipe', 'pipe'],
      ...
      childEvents.on('error', (error: unknown) => {
        const err = error instanceof Error ? error : new Error(String(error))
        trace(`process error=${err.message}`)
        finish(null, err)
      })

      childEvents.on('exit', (code: unknown, signal: unknown) => {
        const exitCode = typeof code === 'number' ? code : null
        const exitSignal = typeof signal === 'string' ? signal : null
        if (done) return
        trace(`exit code=${exitCode ?? 'null'} signal=${exitSignal ?? 'null'}`)
        finish(
          null,
          new Error(
            `Pikafish exited before bestmove (code=${exitCode ?? 'null'}, signal=${exitSignal ?? 'null'}, stdout=${stdoutHistory}, stderr=${stderrHistory})`,
          ),
        )
      })
      ...
      if (evalFilePathForEngine) {
        commands.push(`setoption name EvalFile value ${evalFilePathForEngine}`)
      }
