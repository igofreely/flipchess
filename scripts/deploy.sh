...
#
# 用法：
#   bash scripts/deploy.sh                                    # 默认增量部署（rsync，只传修改文件）
#   bash scripts/deploy.sh --full                             # 全量部署（打包整个项目上传）
#   DEPLOY_HOST=1.2.3.4 bash scripts/deploy.sh                # 自定义主机
#   DEPLOY_PASSWORD=xxx bash scripts/deploy.sh                 # 密码登录（需 sshpass）
#
# 增量部署（默认）：
#   1. 本地构建前端
#   2. rsync 同步变化的文件到远程（跳过 node_modules/NNUE/Pikafish 等大文件）
#   3. 远程重启服务
#   4. 健康检查
#
# 全量部署（--full）：
...
if [[ -n "${DEPLOY_PASSWORD}" ]]; then
    ...
    _rsync() { sshpass -e rsync -e "ssh ${SSH_OPTS}" "$@"; }
else
    _ssh()  { ssh ${SSH_OPTS} "${REMOTE}" "$@"; }
    _scp()  { scp ${SSH_OPTS} "$@"; }
    _rsync() { rsync -e "ssh ${SSH_OPTS}" "$@"; }
fi

MODE_LABEL="增量部署"
if [[ "${FULL_MODE}" == "true" ]]; then MODE_LABEL="全量部署"; fi

echo "╔════════════════════════════════════════════╗"
echo "║   FlipChess 一键部署（${MODE_LABEL}）        ║"
echo "╠════════════════════════════════════════════╣"
...
_rsync -avz --delete \
    ...
    --exclude='.runlogs' \
    --exclude='*.profraw' \
    --exclude='server/data/*.nnue' \
    --exclude='server/data/*.raw.nnue' \
    ./ "${REMOTE}:${DEPLOY_DIR}/"
echo "  ✓ 同步完成"

echo "▶ [3/4] 远程重启服务..."
_ssh bash -s "${DEPLOY_DIR}" "${MYSQL_APP_USER}" "${MYSQL_APP_PASSWORD}" "${MYSQL_APP_DB}" "${PIKAFISH_DIR}" <<'REMOTE_QUICK'
set -euo pipefail
DEPLOY_DIR="$1"
MYSQL_APP_USER="$2"
...
if [[ "${NEED_INSTALL}" == "true" ]]; then
    ...
    echo "  [remote] 依赖无变化，跳过 npm install"
fi
...
if [[ -x "${PIKAFISH_BIN}" ]]; then
    ...
    [[ -f "${PIKAFISH_NNUE}" ]] && export PIKAFISH_EVALFILE_PATH="${PIKAFISH_NNUE}"
...
# nginx（确保配置存在）
if [[ -f /etc/nginx/sites-available/flipchess.conf ]]; then
